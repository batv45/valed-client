...
export default {
  ...
  setup(){
    const register = valedRegister
    const process = ref('')
    const errors = ref([])
    const name = ref('Batuhan OK')
    const email = ref('asd@asd.com')
    const pass = ref('123')
    function submitRegister(){
      process.value = 'register'
      valedRegister(name.value,email.value,pass.value)
          .catch(({data}) => {
    ...
    return {
      register,process,email,pass,submitRegister,errors,name
    }
  }
}
...
<template>
  <div class="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md  w-full space-y-8">
      <div>
        <h2 class="mt-6 text-center text-3xl font-light text-gray-700">Yeni Hesap Oluştur</h2>
      </div>
      <div class="p-4 pt-2 bg-white rounded-lg border border-gray-200 shadow-md sm:p-6 lg:p-8 dark:bg-gray-800 dark:border-gray-700">
        ...
        <div>
          <vd-button type="submit" :loading="process == 'register'" class="w-full ">
            <template #icon>
              <LockClosedIcon class="h-5 w-5 text-gray-100 group-hover:text-gray-400" aria-hidden="true" />
            </template>
            Oturum Aç</vd-button>
        </div>
      </form>
